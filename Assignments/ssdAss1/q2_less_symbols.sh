...
string=$(</dev/urandom tr -dc "_A-Z-a-z-0-9-~@!#$%^&*" | head -c${1:-$PASS_LEN})
...
do
	...
	if [[ $string =~ .*[A-Z].* && $string =~ .*[a-z].* && $string =~ .*[0-9].* && $string =~ .*['~@!#$%^&*'].* && $string =~ ^[A-Za-z].* ]]
	then
		...
		if [[ ! $string =~ .*[a-z][0-9].* && ! $string =~ .*[0-9][a-z].* && ! $string =~ ^['~@!#$%^&*'].* && ! $string =~ .*['~@!#$%^&*']['~@!#$%^&*'].*  && ! $string =~ .*[A-Z]$ ]]   
		then
